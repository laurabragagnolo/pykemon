...
class Battle:
    def __init__(self, trainer, wild_pokemon, type_effectiveness):
    ...
    def turn(self):
        ...
        while True:
            if action == 1:
                ...
                if res == RESULTS[5]:
                    [idx_wild_move,  move_type] = self.getIdxWildPokemonMove()
                    effect = computeEffectiveness(move_type, self.active_pokemon.getType(), self.type_effectiveness)
                    self.wild_pokemon.useMove(idx_wild_move, self.active_pokemon, effect)
    ...
    def fight(self):
        ...
        if len(available_moves) == 0:
            effect_active = computeEffectiveness("normal", self.wild_pokemon.getType(), self.type_effectiveness)  
            idx_active_move = -1
        else:
            i = 0    
            ...
            if choice == i+1:
                return RESULTS[6]
            idx_active_move = map_to_moves[choice]
            effect_active = computeEffectiveness(all_moves[idx_active_move].getType(), self.wild_pokemon.getType(), self.type_effectiveness)
        
        # same for wild pokemon
        [idx_wild_move, move_type] = self.getIdxWildPokemonMove()
        effect_wild = computeEffectiveness(move_type, self.active_pokemon.getType(), self.type_effectiveness)
        
        # check the velocity of the pokemon
        if self.active_pokemon.getBaseStats().getSpeed() > self.wild_pokemon.getBaseStats().getSpeed():
            self.active_pokemon.useMove(idx_active_move, self.wild_pokemon, effect_active)
            if self.wild_pokemon.getC_HP() > 0:
                self.wild_pokemon.useMove(idx_wild_move, self.active_pokemon, effect_wild)
            else:
                return RESULTS[1]
        else:
            self.wild_pokemon.useMove(idx_wild_move, self.active_pokemon, effect_wild)
    ...
    def getIdxWildPokemonMove(self):
        ...
        return [idx_wild_move, all_moves[idx_wild_move].getType()]
